...
void vtkSlicerRos2Logic::RobotCpPubCallback(const std::shared_ptr<geometry_msgs::msg::PoseStamped> msg)
{
  std::cerr << "Robot publisher callback" << std::endl;

}

void vtkSlicerRos2Logic::RobotCpCallback(const std::shared_ptr<geometry_msgs::msg::PoseStamped> msg)
{
  std::cerr << "Robot CP sub callback" << std::endl;
  if (collisionFlag == false){
    geometry_msgs::msg::WrenchStamped wrench; // this is home position - so we need to not send the servo cp any message - should be sending servo cf a null force
    mRobotServoPublisher->publish(wrench);
  }
  else{
    // TRY THIS TOMORROW WITH msg
    mRobotCpPublisher->publish(*msg);
  }
...
void vtkSlicerRos2Logic::SetRobotCPTopic(){

  mRobotCpSubscription
    = mNodePointer->create_subscription<geometry_msgs::msg::PoseStamped>
    ("/arm/measured_cp", 10000, std::bind(&vtkSlicerRos2Logic::RobotCpCallback,
				  this, std::placeholders::_1));
  mRobotCpPublisher
  = mNodePointer->create_publisher<geometry_msgs::msg::PoseStamped>
  ("/arm/servo_cp", 10);
  mRobotServoPublisher
  = mNodePointer->create_publisher<geometry_msgs::msg::WrenchStamped>
  ("/arm/servo_cf", 10);
}
